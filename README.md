...
- **Undiscovered areas**, when turned on, are shaded in see-through gold in their real shape on the zone map, so you can see where to go at a glance. Hovering one names it and, when a Legacy challenge counts it, shows the challenge, its points and how many of the zone's areas are left.
- **Zone completion**: how much of a zone you've done, Guild Wars 2 style. Areas explored, flight paths learned and local reputations at Friendly count for the character; dungeons and the zone's other Legacy objectives count your Legacy progress on the account. It shows in the world map's corner, for the zone you're viewing, with a percentage per zone on continent maps. It can also show in the objective tracker, for the zone you're in (the zone name, a percentage and one row of counts), once you tick that in the map menu. Each can be collapsed, and hovering lists what's left. "What counts" in the same menu leaves out any category you don't care about, and the percentage follows.
