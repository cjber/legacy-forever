...
- `Locales/` — `ns.L`, keyed by the English phrase; the packager fills `Translations.lua` from CurseForge at release.
  `Locales/phrases.txt` is `tools/phrases.py`'s output, pasted into CurseForge's Import localization page.
